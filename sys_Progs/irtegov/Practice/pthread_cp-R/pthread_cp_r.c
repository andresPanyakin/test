    ...
    do {
        ...
        /* trying to open src file */
            ...
            if (srcfile < 0 && errno!=EMFILE) {
                perror(dstpathname);
                return NULL;
            } else if (srcfile < 0 && errno==EMFILE) {
                sleep(1);
            }
        } while (srcfile<0);
        ...
        if (dstfile < 0 && errno != EMFILE) {
            ...
            close(srcfile);
            return NULL;
        } else if (dstfile < 0 && errno == EMFILE) {
            close(srcfile);  /* see comment in the beginning of outer DO block */
            sleep(1);
        }
    } while(dstfile<0);
...
void * copy_directory(void * param) {
   ...
   /* Unreacheable but makes some compilers happy */
   return NULL;
}
